...
1. **서비스가 점검 중이다.** 누가 접속하든 점검 화면만 보인다. 관리자 우회는 `https://tether-d1dab.web.app/?bypass=tether-admin` (localStorage에 기억됨, 해제는 `?bypass=off`).
...
**한계**: 앱 화면 레벨 차단이라 브라우저 콘솔에서 SDK를 직접 호출하면 데이터에 닿는다. 규칙 레벨 차단은 요청당 `get()` 증가와 관리자까지 잠길 위험 때문에 보류했다. 우회 값 `tether-admin`도 클라이언트 번들에 노출된다.
...
| 로그아웃 상태 차단 | 확인 |
| `?bypass=tether-admin` | 앱 진입 확인 (라이브) |
| 우회 기억 / `?bypass=off` | 확인 |
