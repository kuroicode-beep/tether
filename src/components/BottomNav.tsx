...
export function BottomNav({ active, onNavigate }: BottomNavProps) {
  ...
  return (
    <nav
      className="bottom-nav app-fixed-x fixed bottom-0 flex justify-around items-center pt-sm px-xl bg-surface-container-low/80 backdrop-blur-md shadow-sm z-50 rounded-t-xl"
      ...
      {items.map(({ id, badgeKey, icon, label }) => {
        ...
        return (
          <button
            ...
            className={
              isActive
                ? 'bottom-nav-btn relative flex flex-col items-center justify-center bg-primary-container text-on-primary-container rounded-full px-md py-xs scale-95 transition-transform duration-150'
                : 'bottom-nav-btn relative flex flex-col items-center justify-center text-secondary px-md py-xs hover:text-primary transition-colors duration-200'
            }
          >
